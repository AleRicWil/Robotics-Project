...
v2.0 updates:
- Added rad_to_deg utility function for converting joint angles to degrees (for hardware integration).
- Added comments emphasizing use of rad_to_deg before sending to interpreters expecting degrees.
"""
...
def rad_to_deg(q):
    ...
    :param q: list or np.array of angles in radians.
    :return: np.array in degrees.
    Use this before sending q to hardware interpreters that expect degrees (e.g., Robot_Interpreter).
    """
    return np.array(q) * (180.0 / np.pi)