...
from Robot_Interpreter_v1_1 import RobotInterpreter
...
class JointControlUI:
    ...
    def create_composite_section(self, row: int):
        ...
        ttk.Label(composite_frame, text="Speed2:", font=('Arial', 12)).pack(side='left', padx=5)
        ...
        combined_button.pack(side='left', padx=10)
    ...
    def send_composite(self):
        ...
        try:
            ...
            if not all([disp2_str, speed2_str, disp3_str]):
                raise ValueError("Fields empty.")
            disp2 = float(disp2_str)
            speed2 = float(speed2_str)
            disp3 = float(disp3_str)
            was_sent = self.interpreter.send_composite_command(disp2, speed2, disp3, speed2)  # Use speed2 for both
            msg = "Composite sent" if was_sent else "Composite queued"
            color = "green" if was_sent else "blue"
            self.message_label.config(text=f"{msg} for wrist.", foreground=color)
        except ValueError as ve:
