...
// v1.3: Simplified holding logic to global enable/disable.
...
#define STEPR_STEP_PIN 5    // Bottom STEP
#define STEPR_DIR_PIN 6     // Bottom DIR
#define STEPR_DIS_PIN 7    // Bottom DISABLE
...
// NEW: Track last all-idle state for global enable/disable (low overhead)
...
// NEW: Global enable/disable functions
...
void setup() {
  Serial.begin(115200);
  Stepper_Setup();
  Disable_All_Motors();  // Start with all disabled (saves power)
  delay(500);
...
void loop() {
  ...
  // NEW: Global holding check (runs every loop, negligible overhead)
  // - If transitioning from all-idle to not-all-idle: enable all
  // - If transitioning from not-all-idle to all-idle: disable all
...
// NEW: Enable all motors (LOW on disable pins)
void Enable_All_Motors() {
...
// NEW: Disable all motors (HIGH on disable pins)
void Disable_All_Motors() {
...
void Motor_Start(char motor_id, long steps, float rps, int direction) {
  ...
  // REMOVED: Individual enable here; now handled globally in loop
...
void Handle_Stepper_FSM(StepperMotor* motor, MotorState* state, char motor_id) {
  switch (*state) {
    ...
    case MOTOR_DECEL: {
      ...
      if (new_rps <= motor->target_rps) {
        ...
        if (motor->is_blending) {
          ...
          // REMOVED: Redundant enable; global handles it
          if (motor->target_rps <= MAX_COLD_START_RPS) {
    ...
    case MOTOR_STOP:
      // REMOVED: Motor_Disable here; now global in loop
      Serial.print(motor_id);
