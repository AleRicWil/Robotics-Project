...
#define STEPB_DIS_PIN 8    // Bottom DISABLE
#define STEPB_STEP_PIN 12   // Bottom STEP
#define STEPB_DIR_PIN 13    // Bottom DIR

#define STEPR_STEP_PIN 2    // Right STEP
#define STEPR_DIR_PIN 3     // Right DIR
...
const float SERVO_OFFSET_X = 129.5f;
const float SERVO_OFFSET_Y = 138.0f;
...
const float SERVO_OFFSET_Z = 0.0f;
...
const unsigned long UPDATE_INTERVAL_MS = 50UL;  // Fixed update interval for non-blocking motion
...
// Additional holding mode: When true, keeps steppers enabled even if all are idle (persistent torque hold).
// Concept: Provides user-controlled override for applications needing constant holding without motion.
...
void loop() {
  ...
  // Concept: Enables all steppers if any are moving OR hold mode is active.
  // Disables only if all are idle AND hold mode is off.
  // Uses edge detection to avoid repeated calls for efficiency.
  bool current_all_idle = (state_right == MOTOR_IDLE && state_left == MOTOR_IDLE && state_bottom == MOTOR_IDLE);
  bool should_enable = !current_all_idle || hold_mode;
  if (should_enable && all_motors_idle) {
    Enable_All_Motors();
    all_motors_idle = false;
  } else if (!should_enable && !all_motors_idle) {
    Disable_All_Motors();
...
void Servo_Setup() {
  ...
  motor_x.target_angle = SERVO_OFFSET_X;
  motor_x.current_angle = SERVO_OFFSET_X;
  motor_y.target_angle = SERVO_OFFSET_Y;
  motor_y.current_angle = SERVO_OFFSET_Y;
  motor_z.target_angle = SERVO_OFFSET_Z;
  motor_z.current_angle = SERVO_OFFSET_Z;

  Servo_Start('X', motor_x.target_angle, motor_x.max_speed_rps);
  Servo_Start('Y', motor_y.target_angle, motor_y.max_speed_rps);
  Servo_Start('Z', motor_z.target_angle, motor_z.max_speed_rps);
}
...
void Servo_Start(char servo_id, float angle, float speed) {
  ...
  // if (abs(delta) < 0.05f) {  // Small tolerance to ignore negligible moves
...
void Handle_Serial_Input() {
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\n') {
      ...
      if (input.startsWith("Q ")) {
      ...
      else if (input == "H") {  // New: Enable hold mode (persistent enable)
        hold_mode = true;
        Serial.println("Hold mode enabled.");
      } 
      else if (input == "R") {  // New: Disable hold mode (allow auto-disable when idle)
        hold_mode = false;
        Serial.println("Hold mode disabled."); 
      } 
      else {
        ...
        if (id == 'X' || id == 'Y' || id == 'Z') {
          // Servo command
            if (id == 'X') {
              degrees += SERVO_OFFSET_X;
            }
            else if (id == 'Y') {
              degrees += SERVO_OFFSET_Y;
            }
            else if (id == 'Z') {
              degrees += SERVO_OFFSET_Z;
            }
...
// Enhanced Stepper Query – now reports idle state
void Handle_Stepper_Query(char motor_id) {
  StepperMotor* motor;
  MotorState* state;
  bool is_idle;
  ...
  switch (motor_id) {
    case MOTOR_R: motor = &motor_right; state = &state_right; is_idle = (state_right == MOTOR_IDLE); break;
    case MOTOR_L: motor = &motor_left;   state = &state_left;   is_idle = (state_left   == MOTOR_IDLE); break;
    case MOTOR_B: motor = &motor_bottom; state = &state_bottom; is_idle = (state_bottom == MOTOR_IDLE); break;
...
// Enhanced Servo Query – now reports idle state
void Handle_Servo_Query(char servo_id) {
  ServoMotor* motor;
  ServoState* state;
  float offset;
  ...
  switch (servo_id) {
    case SERVO_X: motor = &motor_x; state = &state_x; offset = SERVO_OFFSET_X; is_idle = (state_x == SERVO_IDLE); break;
    case SERVO_Y: motor = &motor_y; state = &state_y; offset = SERVO_OFFSET_Y; is_idle = (state_y == SERVO_IDLE); break;
    case SERVO_Z: motor = &motor_z; state = &state_z; offset = SERVO_OFFSET_Z; is_idle = (state_z == SERVO_IDLE); break;
    default: Serial.println("Invalid ID."); return;
  }

  Serial.print(servo_id);
  Serial.print(" current angle: ");
  Serial.println(motor->current_angle - offset, 1);
